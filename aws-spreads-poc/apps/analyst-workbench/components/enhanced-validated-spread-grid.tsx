...
export function EnhancedValidatedSpreadGrid({ spreadId, spreadVersionId, rows, formulaTrace, provenance }: Props) {
  ...
  const revertSelected = () => {
    ...
    setEditedValues((current) => ({ ...current, [selectedRow.rowId]: initialValues[selectedRow.rowId] || '0' }));
    setStatusMessage(`Reverted ${selectedRow.rowId}.`);
  };

  const focusNextRow = (currentRowId: string) => {
    ...
    <section className="twoCol">
      <div className="panel">
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          <h2>Enhanced validated grid</h2>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            <button onClick={saveAllDirty} disabled={isPending || !dirtyRows.size} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid #2b3759', background: '#17213c', color: dirtyRows.size ? '#8ab4ff' : '#a7b4d6', cursor: dirtyRows.size ? 'pointer' : 'not-allowed' }}>Save all dirty</button>
            <button onClick={revertSelected} disabled={isPending || !selectedRow} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid #2b3759', background: '#17213c', color: '#fbbf24', cursor: selectedRow ? 'pointer' : 'not-allowed' }}>Revert selected</button>
          </div>
        </div>

        <div style={{ overflowX: 'auto', border: '1px solid #2b3759', borderRadius: 14, marginTop: 12 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '220px 220px 180px 140px 120px 120px', minWidth: 1000 }}>
            <div style={{ position: 'sticky', top: 0, padding: 12, borderBottom: '1px solid #2b3759', background: '#17213c', fontWeight: 700 }}>Row ID</div>
            <div style={{ position: 'sticky', top: 0, padding: 12, borderBottom: '1px solid #2b3759', background: '#17213c', fontWeight: 700 }}>Label</div>
            <div style={{ position: 'sticky', top: 0, padding: 12, borderBottom: '1px solid #2b3759', background: '#17213c', fontWeight: 700 }}>FY2025</div>
            <div style={{ position: 'sticky', top: 0, padding: 12, borderBottom: '1px solid #2b3759', background: '#17213c', fontWeight: 700 }}>Origin</div>
            <div style={{ position: 'sticky', top: 0, padding: 12, borderBottom: '1px solid #2b3759', background: '#17213c', fontWeight: 700 }}>Confidence</div>
            <div style={{ position: 'sticky', top: 0, padding: 12, borderBottom: '1px solid #2b3759', background: '#17213c', fontWeight: 700 }}>Action</div>
            ...
            {rows.map((row, index) => {
              ...
              return [
                ...
                <button key={`${row.rowId}-label-${index}`} onClick={() => setSelectedRowId(row.rowId)} style={{ padding: 12, borderBottom: `1px solid ${border}`, background: bg, color: '#eef3ff', textAlign: 'left', cursor: 'pointer', fontWeight: 600 }}>{row.label}</button>,
                ...
                <div key={`${row.rowId}-origin-${index}`} style={{ padding: 12, borderBottom: `1px solid ${border}`, background: bg }}>
                  <span className="badge accent">{row.origin}</span>
                </div>,
                <div key={`${row.rowId}-confidence-${index}`} style={{ padding: 12, borderBottom: `1px solid ${border}`, background: bg }}>
                  <span className="badge good">{row.confidence}</span>
                </div>,
